...
const PinPage = ({ user }) => {
  ...
  const handleImageDownload = async () => {
    try {
      const response = await fetch(pin.image.url, { mode: "cors" });
      const blob = await response.blob();
      const url = URL.createObjectURL(blob);

      const link = document.createElement("a");
      link.href = url;
      link.download = "image.jpg"; // you can customize name here
        ...
        <div className="flex flex-col items-center bg-gray-100 p-4 min-h-screen">
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-lg flex flex-wrap w-full max-w-4xl">
              <div className="w-full md:w-1/2 bg-gray-200 rounded-t-lg md:rounded-l-lg md:rounded-t-none flex flex-col items-center justify-center">
                {pin.image && (
                  <>
                    <img
                      src={pin.image.url}
                    ...
                    <button
                      ...
                      className="mt-3 bg-[#1d87db] text-white py-2 px-4 rounded hover:bg-blue-700 transition"
                    >
                      Download Image
                    </button>
                  </>
                ...
                {pin.tags && pin.tags.length > 0 && (
                  <div className="mt-4 mb-4 px-4 w-full flex flex-wrap justify-start gap-2">
                    {pin.tags.map((tag, i) => (
                      <span
                        key={i}
              ...
              <div className="w-full md:w-1/2 p-6 flex flex-col ">
                <div className="flex items-center justify-between mb-4 ">
                  ...
                  {pin.owner && pin.owner._id === user._id && (
                    <button
                      ...
                      className="bg-[#1d87db] text-white py-1 px-3 rounded"
                    >
                      <MdDelete />
                    </button>
                  )}
                ...
                {edit && (
                  <button
                    style={{ width: "200px" }}
                    className="bg-[#1d87db] text-white py-1 px-3 mt-2 mb-2"
                ...
                <div className="flex items-center mt-4">
                  ...
                  <form className="flex-1 flex" onSubmit={submitHandler}>
                    ...
                    <button
                      type="submit"
                      className="ml-2 bg-[#1d87db] px-4 py-2 rounded-md text-white"
                    >
                      Add+
                    </button>
                  </form>
